...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0"
	...
	<!-- 
	 ...
	 -->
	<xsl:param name="xsd-schema-location" select="'http://www.w3.org/2001/XMLSchema.xsd'"/>
	...
	<xsl:template match="xs:*/@source | xs:*/@schemaLocation">
		<xsl:call-template name="attribute">
			<xsl:with-param name="reference">
		        <xsl:call-template name="adjust-reference">
...
.markup-comment  {color:green}
			...
			<xsl:choose>
				<xsl:when test="$reference">
					<a href="{$reference}">
                        <!--  MOHI 2012-12-03 -->
                        <xsl:choose>
                            <xsl:when test="contains(., '/landscape/schema/')">
                                <xsl:value-of select="concat('http://', substring-after(., '/landscape/schema/'))"/>
                            </xsl:when>
                            <xsl:otherwise>
                                <xsl:value-of select="."/>
                            </xsl:otherwise>
			...
			<xsl:when test="$supports-namespace-axis">
				<xsl:if test="not(../namespace::*[. = $targetNamespace and name(.) =
					...
					<xsl:choose>
						...
						<xsl:otherwise>
                            <xsl:call-template name="adjust-reference">
                                <xsl:with-param name="reference" select="ancestor::xs:schema/xs:import[@namespace=$uri]/@schemaLocation"/>
                            </xsl:call-template>
						</xsl:otherwise>
					</xsl:choose>
				</xsl:if>
    ...
    <xsl:template name="adjust-reference">
        <xsl:param name="reference"/>
		<!--  xsl:choose>
			<xsl:when test="starts-with($reference, 'http://')">
				<xsl:value-of select="concat('http://nkm01873/', substring-after($reference, 'http://'))"/>
			</xsl:when>
			<xsl:otherwise>
